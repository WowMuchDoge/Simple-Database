...
void TableHead::writeToFile(std::string fileName) {
    ...
    if (columns.size() > 0) {
        ...
        for (int i = 0; i < columns[0]->rowLen(); i++) {
            ...
            for (int i = 0; i < columns.size(); i++) {
                if (columns[i]->getTypeName() == "i") {
                    file << ((ColumnHead<int>*)columns[i].get())->getElement(i) << (j == columns.size() - 1 ? "" : " ");
                } else if (columns[i]->getTypeName() == "NSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEE") {
                    file << ((ColumnHead<std::string>*)columns[i].get())->getElement(i) << (j == columns.size() - 1 ? "" : " ");
                } else if (columns[i]->getTypeName() == "d") {
                    file << ((ColumnHead<double>*)columns[i].get())->getElement(i) << (j == columns.size() - 1 ? "" : " ");
                } else if (columns[i]->getTypeName() == "b") {
                    if (((ColumnHead<bool>*)columns[i].get())->getElement(i) == true) {
                        file << "TRUE" << (j == columns.size() - 1 ? "" : " ");
                    } else if (((ColumnHead<bool>*)columns[i].get())->getElement(i) == false) {
